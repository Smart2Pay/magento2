...
class DataAssignObserver extends AbstractDataAssignObserver
{
    ...
    protected static $DETAILS_ARR = array(
    ...
    public function execute( Observer $observer )
    {
        ...
        if( !is_array( $additionalData )
         or empty( $additionalData['selected_country'] )
         or !($country_code = strtoupper( trim( $additionalData['selected_country'] ) ))
         or empty( $additionalData['sp_method'] )
         or !($s2p_method = intval( $additionalData['sp_method'] )) )
            return;
        ...
        if( !($method_details = $configured_methods_instance->getConfiguredMethodDetails( $s2p_method, $environment,
        ...
        $paymentModel->setAdditionalInformation( 'sp_surcharge', (isset( $details_arr['surcharge'] )?$details_arr['surcharge']:0) );
        $paymentModel->setAdditionalInformation( 'sp_fixed_amount', (isset( $details_arr['fixed_amount'] )?$details_arr['fixed_amount']:0) );
        //$paymentModel->setAdditionalInformation( 'sp_transaction', $s2p_transaction->getID() );

        if( !empty( $details_arr['surcharge'] )
         or !empty( $details_arr['fixed_amount'] ) )
        {
            ...
            if( ($total_base_amount = $quote->getBaseGrandTotal()) )
                $total_base_amount -= ($paymentModel->getS2pSurchargeBaseAmount() + $paymentModel->getS2pSurchargeFixedBaseAmount());

            $surcharge_amount = 0;
            if( !empty( $total_amount )
            and (float)$details_arr['surcharge'] != 0 )
                $surcharge_amount = ( $total_amount * $details_arr['surcharge'] ) / 100;
            $surcharge_base_amount = 0;
            if( !empty( $total_base_amount )
            and (float)$details_arr['surcharge'] != 0 )
                $surcharge_base_amount = ($total_base_amount * $details_arr['surcharge']) / 100;

            $surcharge_fixed_amount = 0;
            $surcharge_fixed_base_amount = 0;
            if( (float)$details_arr['fixed_amount'] != 0 )
                $surcharge_fixed_base_amount = $details_arr['fixed_amount'];
