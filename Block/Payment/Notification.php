    ...
    protected function _loadCache()
    {
        ...
        if (!$s2pTransactionLogger->getEnvironment()
         ...
         || empty($api_credentials['site_id']) || empty($api_credentials['apikey'])) {
            $error_msg = 'Couldn\'t load Smart2Pay API credentials for environment ['.
                         $s2pTransactionLogger->getEnvironment().'].';
    ...
    public function sendPaymentDetails(\Magento\Sales\Model\Order $order, $payment_details_arr)
    {
        ...
        try {
            if (!($order_increment_id = $order->getRealOrderId())
             || !($transaction_data = $this->_s2pTransaction->create()->
            ...
            $supportEmail = $this->_helper->getStoreConfig(
                'trans_email/ident_support/email',
                $order->getStoreId()
            );
            $supportName = $this->_helper->getStoreConfig(
