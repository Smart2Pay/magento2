...
class Method extends \Magento\Framework\Model\AbstractModel implements MethodInterface, IdentityInterface
{
    ...
    public function getAllActiveMethods($environment, $params = false)
    {
        ...
        if (!isset($params['method_ids']) || !is_array($params['method_ids'])) {
            $params['method_ids'] = false;
        }
        if (empty($params['order_by']) || !in_array($params['order_by'], [ 'display_name', 'method_id' ])) {
            $params['order_by'] = 'display_name';
        }

        // we received an empty array of ids, so we should return empty result...
        if (is_array($params['method_ids']) && empty($params['method_ids'])) {
            return [];
        }
        ...
        if (!empty($method_ids_arr)) {
            $collection->addFieldToFilter('method_id', [ 'in' => $method_ids_arr ]);
        }
        ...
        while (($method_obj = $collection->fetchItem())
            ...
            if (!empty($params['include_countries'])) {
                $return_arr[$method_arr['method_id']]['countries_list']
                    = $this->getCountriesForMethod($method_arr['method_id'], $environment);
            } else {
                $return_arr[$method_arr['method_id']]['countries_list'] = [];
            }
        }
    ...
    public function saveMethodsFromSDKResponse($methods_arr, $environment)
    {
        ...
        $s2pLogger->write(
            'Updating '.count($methods_arr).' methods for environment '.$environment.' from SDK response.',
            'SDK_methods_update'
        );
        ...
        try {
            ...
            $s2pLogger->write(
                'Couldn\'t delete existing methods from database: '.$e->getMessage(),
                'SDK_methods_update'
            );
        ...
        foreach ($methods_arr as $method_arr) {
            ...
            if (!($db_method = $my_resource->insertOrUpdate($method_arr['id'], $environment, $row_method_arr))) {
                $s2pLogger->write(
                    'Error saving method details in database (#'.$method_arr['id'].').',
                    'SDK_methods_update'
                );
            ...
            if (!empty($method_arr['countries']) && is_array($method_arr['countries'])) {
                if (true !== ($error_msg = $country_method_obj->updateMethodCountries(
                    $method_arr['id'],
                    $method_arr['countries'],
                    $environment,
                    [ 'delete_before_update' => false ]
                ))) {
                    $s2pLogger->write(
                        'Error saving method countries in database (#'.$method_arr['id'].').',
                        'SDK_methods_update'
                    );
